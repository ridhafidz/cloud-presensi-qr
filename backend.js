...
function doGet(e) {
  ...
  if (path === "student") {
    return HtmlService.createHtmlOutputFromFile('mahasiswa')
      .setTitle('Presensi Mahasiswa - QR Checkin')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  }
...
function processCheckin(body) {
  if (!body.user_id || !body.qr_token || !body.course_id || !body.session_id) return { ok: false, error: "missing_field" };
  ...
  if (!tokenExists) return { ok: false, error: "token_invalid" };
  if (!tokenValid) return { ok: false, error: "token_expired" };
  ...
  return { ok: true, data: { presence_id: presenceId, status: "checked_in" } };
}

function processGetStatus(params) {
  if (!params.user_id || !params.course_id || !params.session_id) return { ok: false, error: "missing_parameter" };

  const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName("presence");
  const data = sheet.getDataRange().getValues();

  for (let i = data.length - 1; i > 0; i--) {
    if (data[i][1] === params.user_id && data[i][3] === params.course_id && data[i][4] === params.session_id) {
      return { ok: true, data: {
...
function processAccelBatch(body) {
  if (!body.device_id || !body.data || !Array.isArray(body.data)) return { ok: false, error: "invalid_batch_data" };
  ...
  return { ok: true, data: { saved_records: rows.length } };
}

function processGPS(body) {
  if (!body.device_id || !body.lat || !body.lng) return { ok: false, error: "missing_gps_data" };
  ...
  return { ok: true, data: { status: "recorded", timestamp: recordedAt } };
}

function processGetGPSMarker(params) {
  if (!params.nim) return { ok: false, error: "missing_nim" };

  const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName("gps");
  const data = sheet.getDataRange().getValues();

  for (let i = data.length - 1; i > 0; i--) {
    // Cari data yang Device ID-nya diawali dengan NIM yang diketik dosen
    if (String(data[i][0]).startsWith(params.nim)) {
      return { ok: true, data: {
        lat: data[i][1],
        lng: data[i][2],
        ts: data[i][6]
  ...
  return { ok: false, error: "data_not_found" };
}

function processGetGPSPolyline(params) {
  if (!params.device_id) return { ok: false, error: "missing_device_id" };
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] !== params.device_id) continue;
    const recordTime = new Date(data[i][6]);
    if (fromDate && recordTime < fromDate) continue;
    if (toDate && recordTime > toDate) continue;

    route.push({ lat: data[i][1], lng: data[i][2], ts: data[i][6] });
  }

  return { ok: true, data: { device_id: params.device_id, total_points: route.length, polyline: route } };
}
...
function processGetAccelLatest(params) {
  if (!params.nim) return { ok: false, error: "missing_nim" };

  const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName("accel");
  const data = sheet.getDataRange().getValues();
  
  // Cari dari bawah ke atas agar mendapat data terbaru
  for (let i = data.length - 1; i > 0; i--) {
    // Cari data yang Device ID-nya diawali dengan NIM yang diketik dosen
    if (String(data[i][0]).startsWith(params.nim)) {
      return { ok: true, data: { 
